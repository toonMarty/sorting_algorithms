...
/**
 * helper_num - a function that turns
 * an integer to  a card value
 * @head_tmp1: a pointer to the structure
 * Return: integer
 */
...
/**
 * num_sort - a function that sorts a doubly linked list
...
void num_sort(deck_node_t **list)
{
	...
	{k = head_tmp1->card->kind;
		...
		{
			aux_num1 = helper_num(head_tmp1);
			aux_num2 = helper_num(head_tmp1->next);
			flag = 0;
			head_tmp2 = head_tmp1;
			while (head_tmp2 && head_tmp2->card->kind == k && aux_num1 > aux_num2)
			{
				...
				if (!head_tmp2)
					break;
				aux_num1 = helper_num(head_tmp2);
				aux_num2 = helper_num(head_tmp2->next);
			}
			if (flag == 0)
...
/**
 * kind_sort - a function that sorts a doubly linked list
 * @list: a double pointer to  the head node
 * Return: nothing
 */
...
/**
 * sort_deck - a function that sorts a
 * deck of cards
